...
# USAGE: NOT TO BE RUN
# REQUIREMENT : NONE
# DESCRIPTION : A major feature of this librarty is the command timed_run("cmd", time ), which runs the command "cmd" with a timeout value set to be "time".
#		The function returns 1 if the command is timed out, else it returns 0
#		However, in order to check whether the command is returned with some output message or error message,
#		the function get_recent_outstr() or get_recent_errstr() can be called.    
# <END_DESCRIPTION>
